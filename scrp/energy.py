...
from .models import FlightIntention
...
def _resolve_fi_hover_power(fi: FlightIntention) -> float:
    ...
    Raises ValueError when none of the above is satisfied.
    """
    if fi.P_hover > 0.0:
        return fi.P_hover
...
def compute_E_cruise(fi: FlightIntention) -> Optional[float]:
    """Estimate cruise energy in Wh using the physics-based power model.

    Returns None when propulsion geometry (mtow_kg, num_rotors,
    propeller_diameter_m) is not provided — energy calculation is skipped
    and the caller should not apply a SoC check.
    """
...
def compute_SoC_remaining(
    ...
    t_dep_star: float,
    t_slot_start: float,
    E_cruise: Optional[float],
) -> Optional[float]:
    """Compute SoC remaining after flight and hover wait.

    Returns None when E_cruise is None (propulsion geometry absent).
    ...
    airspace to when its slot opens; landing-phase duration is excluded
    so the drone is not modelled as hovering while already on the ground.
    """
    if E_cruise is None:
        return None
