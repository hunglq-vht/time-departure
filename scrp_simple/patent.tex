...
\begin{equation}
  d_p = t^*_{\mathrm{touch},p} - \tland - \lane - \tto - t_{\mathrm{des}}.
  \label{eq:c4}
\end{equation}
...
\newpage
\appendix
\section{Appendix A: Theoretical Foundations of the Fixed-Point Algorithm
         and Conditions Under Which Mixed-Integer Programming Is Required}
% =============================================================================

\subsection*{A.1\quad Why Monotone Fixed-Point Iteration Is Applicable Here}

\paragraph{A.1.1\quad The structural condition: monotonicity.}
The algorithm's correctness and efficiency rest on a single structural
property of each constraint function $f_k$:

\begin{definition}[Monotone constraint function]
A function $f\colon \mathbb{R}_{\ge 0}\to\mathbb{R}_{\ge 0}$ is a
\emph{monotone constraint function} if $d \le d'$ implies $f(d) \le f(d')$.
\end{definition}

Intuitively, if a departure delay of $d$ seconds is needed to clear a
given set of conflicts, then a delay of $d' > d$ seconds — which shifts
\emph{all} downstream time-stamps uniformly forward — cannot create
\emph{new} conflicts that $d$ did not already address, and may clear
additional ones.  This is the physical content of monotonicity.

\paragraph{A.1.2\quad Monotonicity of each individual constraint.}

\begin{proposition}
Under the UAM model of Section~6, each of $f_1, f_2, f_3, f_4$ is
monotone non-decreasing.
\end{proposition}

\begin{proof}
Let $d \le d'$.  We show $f_k(d) \le f_k(d')$ for each $k$.

\smallskip
\textbf{$f_1$ (lane separation).}
With departure delay $d$, the new drone's entry time into segment $s$ is
$t_i(d) = t_{\mathrm{des}} + d + \Delta_s^{\mathrm{new}}$.
Since $\Delta_s^{\mathrm{new}}$ is fixed by the route and $d' \ge d$,
we have $t_i(d') \ge t_i(d)$.
The minimum-follow constraint $t_i \ge t_j + h_{\min}$ becomes
\emph{easier} to satisfy as $t_i$ grows; similarly, the minimum-lead
constraint $t_i + L/v_i + h' \le t_j$ is either already satisfied by
the same ordering or it ceases to be satisfiable, in which case the
follower ordering (larger $t_i$) is chosen.
In either case the required delay can only stay the same or decrease as
$d$ increases.
Therefore $f_1(d') \ge d' \ge d$, and by the definition of $f_1$ as
the max over all segments, $f_1$ is non-decreasing.

\smallskip
\textbf{$f_2$ (takeoff airspace).}
For delay $d$, the proposed takeoff start is $t_{\mathrm{dep}}(d) =
t_{\mathrm{des}} + d$.
The \textsc{Advance} primitive returns the smallest $t^* \ge
t_{\mathrm{dep}}(d)$ such that $[t^*, t^*+\tto)$ is conflict-free.
Since $t_{\mathrm{dep}}(d') \ge t_{\mathrm{dep}}(d)$ and
$\textsc{Advance}(t, \delta, \mathcal{W}) \ge t$ is non-decreasing in
$t$ (moving $t$ forward can only keep or advance the free slot), we
obtain $f_2(d') \ge f_2(d)$.

\smallskip
\textbf{$f_3$ (landing airspace).}
Identical argument to $f_2$: the descent window start $\twpN(d)$ grows
with $d$; \textsc{Advance} is monotone in its first argument; and the
back-calculation $d' = \twpN^* - \tto - \lane - t_{\mathrm{des}}$ is
an affine (hence monotone) function of $\twpN^*$.

\smallskip
\textbf{$f_4$ (pad occupancy).}
The touchdown time $\ttouch(d)$ grows with $d$.  For each pad $p$, the
earliest free touchdown $t^*_{\mathrm{touch},p} \ge \ttouch(d)$ can only stay or
increase as $\ttouch(d)$ increases (a forward point can miss the same
or more occupation windows).  The back-calculation to departure delay is
again affine.  Taking the minimum over pads preserves monotonicity.
\end{proof}

\paragraph{A.1.3\quad Knaster--Tarski and the orbit from zero.}
Because the constraint functions map the bounded complete lattice
$([0, T_{\max}], \le)$ to itself monotonically, the Knaster--Tarski
fixed-point theorem guarantees:

\begin{enumerate}[label=(\roman*)]
  \item $F = f_4 \circ f_3 \circ f_2 \circ f_1$ has a \emph{least
    fixed point} $d^*$.
  \item Starting from the bottom element $d_0 = 0$, the orbit
    $d_{k+1} = F(d_k)$ is non-decreasing and converges to $d^*$
    in finitely many steps.
\end{enumerate}

The orbit cannot overshoot: because $F$ is monotone and $d^*$ is the
least fixed point, $d_k \le d^*$ for all $k$ (induction: $d_0=0 \le
d^*$; if $d_k \le d^*$ then $F(d_k) \le F(d^*) = d^*$ by
monotonicity).

\paragraph{A.1.4\quad Why convergence is fast in practice.}
Each iteration $d_{k+1} = F(d_k)$ moves $d$ to the next
\emph{window endpoint} that is relevant to the current delay value.
The total number of distinct window endpoints across all four constraint
pools is at most $2P$ (two endpoints per approved plan).
In typical UAM operations $P$ is small (tens to hundreds of active
plans within a scheduling horizon), and many window endpoints lie
outside the narrow time range affected by the new request.
Empirically, 2--4 iterations suffice in practice.

\medskip
By contrast, a Mixed-Integer Program over the same problem requires a
branch-and-bound search tree whose worst-case size is
$\mathcal{O}(2^{B})$ where $B$ is the number of binary variables
(one per ordering decision per conflict per constraint).  Even though
modern solvers prune aggressively, they cannot exploit monotonicity and
therefore cannot guarantee the 2--4 iteration bound.
The benchmark results in Table~\ref{tab:benchmark} confirm this gap.

\begin{table}[h]
\centering
\caption{Measured wall-clock time (ms): Fixed-Point vs MIP (CBC/PuLP).
         Single new request; 10 repeats each.}
\label{tab:benchmark}
\begin{tabular}{rrrr}
\toprule
$P$ (approved plans) & FP mean (ms) & MIP mean (ms) & MIP\,/\,FP ratio \\
\midrule
  5 & 0.17 &   6.9 & 41$\times$ \\
 10 & 0.27 &   9.0 & 34$\times$ \\
 20 & 0.48 &  13.4 & 28$\times$ \\
 40 & 0.87 &  26.0 & 30$\times$ \\
 80 & 1.70 &  88.9 & 52$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent Both solvers return the same optimal delay $d^*$ (verified to
within 1-second tolerance across all test cases), confirming that the
fixed-point algorithm achieves global optimality without the overhead of
MIP.

% ─────────────────────────────────────────────────────────────────────────────

\subsection*{A.2\quad Conditions Under Which Monotone Fixed-Point Iteration
             Fails and MIP Is Required}

The fixed-point approach is applicable \emph{if and only if} each
constraint function $f_k$ is monotone non-decreasing in the departure
delay $d$.  Monotonicity can break down in the following operationally
relevant scenarios.

\paragraph{A.2.1\quad Variable or optimisable cruise speed.}
In the present invention the new drone's per-segment speed $v_i$ is
\emph{fixed} by the request.  If speed is a second decision variable
(e.g., a trajectory-optimising planner that may slow down to avoid a
conflict), then the minimum headway $h_{\min}(v_i, v_j, L)$ depends on
the chosen $v_i$, coupling the speed and delay decisions
non-linearly.  The constraint function over the joint space
$(d, v_i)$ is no longer monotone in either coordinate alone; even a
higher $d$ might require a lower $v_i$ to satisfy lane separation,
while a lower $v_i$ changes the lane time and thus the airspace
conflict window.

\begin{figure}[h]
\centering
\begin{tikzpicture}[>=Stealth, scale=0.85]
  % Feasible region for (d, v) -- non-convex
  \draw[->] (0,0) -- (6.5,0) node[right]{$d$ (delay)};
  \draw[->] (0,0) -- (0,4.5) node[above]{$v_i$ (speed)};

  % Non-convex feasible region
  \fill[blue!10, draw=blue!50, rounded corners=4pt]
    (1.0,3.5) -- (3.5,3.5) -- (3.5,2.2) -- (5.5,2.2) --
    (5.5,0.6) -- (1.0,0.6) -- cycle;

  \node[blue!70] at (3.2,2.0) {\small feasible region};

  % Monotone fixed-point path (wrong direction)
  \draw[->, red!80, thick, dashed]
    (1.2,1.2) -- (2.0,1.2) node[below, font=\tiny]{C1 push} --
    (2.0,2.8) node[right, font=\tiny]{C2} -- (3.0,2.8);
  \node[red!80, font=\footnotesize] at (4.5,3.0) {path with fixed $v_i$};

  % Infeasible "pocket"
  \fill[red!15, draw=red!60, rounded corners=2pt]
    (1.5,1.5) rectangle (3.2,2.0);
  \node[red!60, font=\tiny] at (2.35,1.75) {infeasible pocket};
\end{tikzpicture}
\caption{With variable speed $v_i$, the feasible region in $(d, v_i)$
space can be non-convex and contain infeasible ``pockets'' that a
monotone orbit (fixing $v_i$) cannot route around.}
\end{figure}

\paragraph{A.2.2\quad Route choice (multi-path requests).}
If the new flight plan may choose among several alternative routes
$\{\rho_1, \rho_2, \ldots\}$, the set of shared lane segments —
and hence the active constraints — changes discretely with the route
choice.  There is no single monotone function $f_1$ that captures all
routes simultaneously; instead, $f_1$ becomes a minimum over route-
specific functions, each with a different domain of validity.  The
combined function can be non-monotone: a larger delay might eliminate a
conflict on route $\rho_1$ while exposing a previously irrelevant
conflict on $\rho_2$, causing the composed map to decrease.

\begin{example}
Route $\rho_1$ shares segment $s_1$ with plan $P_A$ (window $[10, 50)$\,s).
Route $\rho_2$ shares segment $s_2$ with plan $P_B$ (window $[60, 100)$\,s).
At $d = 5$\,s the cheapest route is $\rho_1$ (requires $d = 40$\,s).
At $d = 40$\,s the cheapest route switches to $\rho_2$ (requires
$d = 60$\,s).  Back at $d = 60$\,s, $\rho_1$ is again conflict-free
at $d = 40$\,s $<$ 60\,s.
The composed multi-route function is not monotone; MIP with binary
route-selection variables is required.
\end{example}

\paragraph{A.2.3\quad Bidirectional lanes (head-on conflict resolution).}
The present invention assumes all drones traverse a shared lane segment
in the \emph{same direction}.  In a bidirectional corridor, a head-on
conflict between the new drone (direction $+$) and an approved drone
(direction $-$) cannot be resolved by delaying the new drone within the
segment: the approved drone's position grows \emph{toward} the new
drone as time increases, making delay counter-productive.  Resolution
requires either (a)~choosing a time slot in which the segment is free
in both directions, or (b)~rerouting.  Neither option is captured by a
monotone function of $d$ alone.

\paragraph{A.2.4\quad Simultaneous batch scheduling without priority ordering.}
Algorithm~2 (Batch Resolution) resolves requests \emph{sequentially} in
priority order, which preserves monotonicity at each step (the pool
$\mathcal{A}'$ is fixed when each individual request is resolved).
If, however, the goal is to \emph{jointly} optimise the delays of all
$m$ requests — for example, to minimise the total delay
$\sum_{i=1}^{m} d_i$ or the makespan $\max_i d_i$ — then the
constraint functions of different requests become coupled:
\[
  f_k^{(i)}(d_1, \ldots, d_m) \text{ depends on } d_j \text{ for } j \ne i.
\]
This coupling destroys the product-lattice monotonicity and requires a
joint MIP with $m \cdot B$ binary variables (where $B$ is the binary
count per single request).

\begin{example}[Optimality gap of sequential priority resolution]
Two requests $r_1, r_2$ both want to depart at $t=0$ from the same
vertiport whose airspace is blocked on $[0,10)$.
Priority ordering gives $r_1$ the slot at $t=10$ (delay $d_1=10$\,s)
and forces $r_2$ to $t=20$ (delay $d_2=20$\,s); total delay $=30$\,s.
A joint MIP finds $d_1=10$, $d_2=10$ (different pads, no further
airspace conflict); total delay $=20$\,s — 33\% better.
\end{example}

\paragraph{A.2.5\quad Time-dependent window durations (weather, wind).}
If the duration of a flight phase depends on the actual departure time
— for example, head-wind increases $\lane$ when departing early but
tail-wind decreases it when departing late — then the touchdown time
$\ttouch(d)$ is a non-linear, potentially non-monotone function of $d$.
A faster tailwind at large $d$ could make $\ttouch(d)$ smaller than at
a smaller $d$, violating the monotone ordering assumed in $f_3$ and
$f_4$.

\paragraph{A.2.6\quad Lane capacity $> 1$ with joint separation constraints.}
The present invention treats each lane segment as capacity-1 (one drone
at a time, subject to headway).  With capacity $K \ge 2$, the
new drone must maintain $h_{\min}$ from the nearest $K$ drones.
When the $K$-th nearest approved drone's entry time is itself a function
of which earlier drones are present (e.g., if those drones also had
required delays), the constraint function $f_1$ couples multiple
approved plans simultaneously, potentially producing a non-monotone
map.

\paragraph{A.2.7\quad Objective other than individual minimum delay.}
If the objective is not $\min d$ but, for instance:
\begin{itemize}
  \item $\min \sum_i d_i$ (total delay across all requests),
  \item $\min \max_i d_i$ (fairness / makespan minimisation),
  \item $\max$ throughput (number of approved plans per hour),
\end{itemize}
then a single monotone iteration on individual requests cannot capture
the coupling between requests.  A MIP with a joint objective is
necessary.

% ─────────────────────────────────────────────────────────────────────────────

\subsection*{A.3\quad Summary: Applicability Decision Table}

Table~\ref{tab:applicability} provides a concise guide for practitioners.

\begin{table}[h]
\centering
\caption{Conditions determining whether fixed-point or MIP is required.}
\label{tab:applicability}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{p{5.8cm} c c}
\toprule
\textbf{Scenario} & \textbf{Fixed-Point} & \textbf{MIP} \\
\midrule
Fixed speed, fixed route, individual $\min d$       & \checkmark & optional \\
Variable speed as decision variable                  & $\times$   & \checkmark \\
Multi-route choice                                   & $\times$   & \checkmark \\
Bidirectional lanes                                  & $\times$   & \checkmark \\
Batch: sequential by priority, individual $\min d$   & \checkmark & optional \\
Batch: joint $\min \sum d_i$ or $\min \max d_i$     & $\times$   & \checkmark \\
Time-dependent wind / weather affecting phase times  & $\times$   & \checkmark \\
Lane capacity $> 1$, joint separation constraints    & $\times$   & \checkmark \\
Stochastic arrival times (chance constraints)        & $\times$   & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\noindent
For all scenarios marked \checkmark in the Fixed-Point column, the
algorithm of the present invention is both \emph{complete} (finds the
globally optimal $d^*$) and \emph{efficient} (typically 2--4
iterations, $\mathcal{O}(P)$ per iteration).  For scenarios marked
$\times$, the monotonicity condition is violated; a MIP or other
combinatorial method is required to guarantee global optimality.

The scope of the present invention is the first category: UAM networks
with fixed-route, fixed-speed flight plans resolved individually or
sequentially by priority, which is the dominant use case in current
generation UTM systems.
